...
# 2. 카테고리 별 가장 비싼 메뉴 가격 조회
select *
from tbl_menu main
where main.menu_price = (
    select max(sub.menu_price)
    from tbl_menu sub
...
### 인라인 뷰 (INLINE VIEW)
# - FROM 절에 작성된 서브쿼리
# - 서브쿼리의 결과 집합(ResultSet)을 테이블 처럼 활용
