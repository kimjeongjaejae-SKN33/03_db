...
select
    menu_name as 메뉴명,
    menu_price as "메뉴 가격", # 쌍따옴표를 이용해서 띄어쓰기 된 컬럼 묶기
...
# 컬럼명 별칭 as : ResultSet에 표기되는 컬럼명에 별칭(alias) 부여 가능
# ===================================================
...
select
    menu_name as 메뉴명,
    concat('금', menu_price, '원') as "메뉴 가격(원)" # 작은 따옴표
from
    tbl_menu;
...
# - 특정 칼럼을 기준으로 삼아서 오름차순(ASC), 내림차순(DESC) 지정
...
# 문자열(유니코드 순서), 날짜(과거~미래)도 정렬기준이 될 수 있다.
# 메뉴명 오름차순 정렬
select
    menu_name
from
...
select
    menu_name # from -> select -> order by 캐싱
from
    tbl_menu
order by
    menu_price asc;
...
# 3) 같은 카테코리 코드를 지닌 행 내에서 가격 내림차순 정렬 수행
...
# 메뉴 테이블에서 가격이 10,000원 이상, 20,000 원 이하인
# 메뉴의 메뉴명, 가격, 카테고리 코드를
# 가격 내림차순으로 조회
select
    menu_name,
...
where
    ...
    # menu_name like '%쥬스'; # '쥬스' 포함된 메뉴 조회
...
# 카테고리 테이블에서 ref_category_code 가 null 인 행 조회
select
    *
from
    tbl_category
...
# limit offset, n : offset 행 만큼 건너뛰고 n 행 만큼 조회
