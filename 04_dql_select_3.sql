...
# count(컬럼명) : 지정된 컬럼 값 중 null 인 컬럼을 제외한 행의 개수
select * from tbl_category;
select
    count(*),
    count(tbl_category.ref_category_code)
from
    tbl_category;
...
### group by 사용 시 주의사항
# 1. null도 별도 그룹으로 묶임
# 2. select 저에는 group by 기준이 된 컬럼 + 그룹 함수만 작성 가능
select
    ref_category_code
...
# 각 그룹에서 orderable_status가 같은 행기리 2차 그룹화
select
    category_code,
    orderable_status,
...
where
    m.orderable_status =  'Y' and m.category_code in (4,10)
group by
    c.category_name;
...
group by
    ref_category_code
having
    count(*) >= 3 # and ref_category_code is not null -> where 절에 쓰는 것을 권장 효율
order by
    count(*) asc;
